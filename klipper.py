...
def processmsg():
#     print("Processmsg")
#     print(f"Checking {printer} {printerip}")
     try:
       output = urllib.request.urlopen(f"http://{printerip}:7125/printer/objects/query?webhooks&virtual_sdcard&print_stats").read()
#       print("1")
       data = json.loads(output)
       #print(data)
       stage = data['result']['status']['webhooks']['state']
       ...
       if stage != "ready":
         ...
         printstats= data['result']['status']['print_stats']['state']
         #print(printstats)
         if printstats == "complete":
             setcolor("slowg")
         elif printstats == "standby":
             setcolor("blue")
         elif printstats == "paused":
             setcolor("purple")
         elif printstats == "error":
             setcolor("red")
         elif printstats == "printing":
             setcolor("green")
         else:
             setcolor("black")
     except Exception as e:
